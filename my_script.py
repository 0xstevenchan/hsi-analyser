...
def string_to_unix(year_month_day, hour_minute_second=''):
    if hour_minute_second:
        date_time = datetime.strptime(f"{year_month_day} {hour_minute_second}", '%Y-%m-%d %H:%M:%S')
    else:        
...
def list_to_multiindex(index=list, sep=','):
    tuples = []
    for i in index:
        if isinstance(i, str):
            if i.startswith('(') and i.endswith(')'):
                ...
                i = tuple(i.split(sep))
        tuples.append(i)
    if all(isinstance(i, tuple) for i in tuples):
        tuples = pd.MultiIndex.from_tuples(tuples)
    return tuples
